...
import { isEqualPath } from 'src/utils/is-equal-path';

// ----------------------------------------------------------------------

//  Checks if an anchor element is valid for triggering the progress bar.
function isValidAnchor(element: HTMLAnchorElement | null): boolean {
...
function useProgressBar() {
  ...
  useEffect(() => {
    ...
    const handleNavigation = (newUrl: string) => {
      try {
        if (newUrl && !isEqualPath(newUrl, currentUrlRef.current)) {
    ...
    const patchHistoryMethod = (method: 'pushState' | 'replaceState') => {
      ...
      window.history[method] = new Proxy(originalMethod, {
        apply: (target, thisArg, args: Parameters<typeof originalMethod>) => {
          const newUrl = args[2];
